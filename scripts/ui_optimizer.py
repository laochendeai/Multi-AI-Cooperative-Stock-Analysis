...
class UIOptimizer:
    ...
    def __init__(self):
        ...
        self.ui_file = self.project_root / "final_ui.py"
        self.backup_dir = self.project_root / "backups"
        self.optimized_file = self.project_root / "final_ui_optimized.py"
    ...
    def create_backup(self):
        ...
        backup_file = self.backup_dir / f"final_ui_backup_{timestamp}.py"
        
        if self.ui_file.exists():
            shutil.copy2(self.ui_file, backup_file)
    ...
    def analyze_current_ui(self):
        ...
        try:
            ...
            analysis = {
                ...
                'gradio_components': len(re.findall(r'gr\.\w+\(', content)),
                'tabs_count': len(re.findall(r'gr\.Tab\(', content)),
                'rows_count': len(re.findall(r'gr\.Row\(', content)),
                'columns_count': len(re.findall(r'gr\.Column\(', content)),
                'buttons_count': len(re.findall(r'gr\.Button\(', content)),
                'textboxes_count': len(re.findall(r'gr\.Textbox\(', content)),
                'html_components': len(re.findall(r'gr\.HTML\(', content)),
                'markdown_components': len(re.findall(r'gr\.Markdown\(', content)),
                'file_size_kb': round(len(content.encode('utf-8')) / 1024, 2)
            }

            print("📊 当前UI分析结果:")
            print(f"   📄 文件路径: {self.ui_file}")
            print(f"   📏 总行数: {analysis['total_lines']}")
            print(f"   📦 文件大小: {analysis['file_size_kb']} KB")
            print(f"   🔧 Gradio组件总数: {analysis['gradio_components']}")
            print(f"   📑 标签页数量: {analysis['tabs_count']}")
            print(f"   📐 行布局数量: {analysis['rows_count']}")
            print(f"   📊 列布局数量: {analysis['columns_count']}")
            print(f"   🔘 按钮数量: {analysis['buttons_count']}")
            print(f"   📝 文本框数量: {analysis['textboxes_count']}")
            print(f"   🌐 HTML组件: {analysis['html_components']}")
            print(f"   📋 Markdown组件: {analysis['markdown_components']}")

            # 计算复杂度评分
            complexity_score = (
                analysis['tabs_count'] * 3 +
                analysis['rows_count'] * 2 +
                analysis['columns_count'] * 2 +
                analysis['gradio_components']
            )

            print(f"\n📈 UI复杂度评分: {complexity_score}")
            ...
            # 优化建议
            print(f"\n💡 优化建议:")
            if analysis['tabs_count'] > 5:
                print(f"   • 标签页过多({analysis['tabs_count']}个)，建议合并相关功能")
            if analysis['rows_count'] > 20:
                print(f"   • 行布局过多({analysis['rows_count']}个)，建议使用更紧凑的布局")
            if analysis['total_lines'] > 500:
                print(f"   • 文件过长({analysis['total_lines']}行)，建议模块化拆分")
    ...
    def generate_optimized_ui(self):
        """生成优化后的UI代码"""
...
'''
        ...
        print(f"✅ 优化版UI已生成: {self.optimized_file}")
        return self.optimized_file
    ...
    def apply_optimization(self):
        ...
        # 3. 生成优化版本
        optimized_file = self.generate_optimized_ui()
        if not optimized_file:
            return False
        
        # 4. 替换原文件
        try:
            shutil.copy2(optimized_file, self.ui_file)
            print(f"✅ UI优化完成！原文件已备份到: {backup_file}")
            print(f"📝 优化版本已应用到: {self.ui_file}")
            return True
        except Exception as e:
            print(f"❌ 应用优化失败: {e}")
            return False
    ...
    def rollback_optimization(self):
        """回滚UI优化"""
        # 查找最新的备份文件
        backup_files = list(self.backup_dir.glob("final_ui_backup_*.py"))
        if not backup_files:
            print("❌ 没有找到备份文件")
            return False
        ...
        try:
            shutil.copy2(latest_backup, self.ui_file)
            print(f"✅ 已回滚到备份版本: {latest_backup}")
            return True
        except Exception as e:
            print(f"❌ 回滚失败: {e}")
            return False
...
def main():
    ...
    parser = argparse.ArgumentParser(description="TradingAgents UI优化工具")
    parser.add_argument("action", choices=["analyze", "optimize", "rollback", "generate"],
                       help="执行的操作")
    ...
    if args.action == "analyze":
        optimizer.analyze_current_ui()
    elif args.action == "optimize":
        optimizer.apply_optimization()
    elif args.action == "rollback":
        optimizer.rollback_optimization()
    elif args.action == "generate":
        optimizer.generate_optimized_ui()
